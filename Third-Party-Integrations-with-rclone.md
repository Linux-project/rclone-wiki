...
[rclone4pi](https://github.com/pageauc/rclone4pi) includes an automated bash install script that can run from a [curl command](https://github.com/pageauc/rclone4pi/wiki#quick-install) or [manually](https://github.com/pageauc/rclone4pi/wiki#manual-install) onto a Raspberry Pi computer.  There is also [Wiki](https://github.com/pageauc/rclone4pi/wiki) instructions. A sample rclone-sync.sh script that can be added as a [crontab](https://github.com/pageauc/rclone4pi/wiki#how-to-automate-rclone) and prevents multiple instances of the script from running to avoid problems if one job does not finish before the next scheduled cron. Instructions are provided for setting up a [Cloud Storage Service](https://github.com/pageauc/rclone4pi/wiki#how-to-configure-a-remote-storage-service)